...
class LongitudinalMpc:
  ...
  def update(self, radarstate, personality=log.LongitudinalPersonality.standard, stop_distance=STOP_DISTANCE):
    ...
    # The solver is generated with STOP_DISTANCE. Shift the obstacle by the
    # inverse delta to preserve dragonpilot's adjustable stopped distance.
    stop_distance_offset = STOP_DISTANCE - stop_distance
    lead_0_obstacle = lead_xv_0[:,0] + get_stopped_equivalence_factor(lead_xv_0[:,1]) + stop_distance_offset
    lead_1_obstacle = lead_xv_1[:,0] + get_stopped_equivalence_factor(lead_xv_1[:,1]) + stop_distance_offset
